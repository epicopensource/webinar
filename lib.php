 ...
 * Finds all webinar notifications that have yet to be mailed out, and mails them.
 */
function webinar_cron()
{
    global $CFG, $USER, $DB;

    $signupsdata = webinar_get_unmailed_reminders();
    if (!$signupsdata) {
        echo "\n".get_string('noremindersneedtobesent', 'webinar')."\n";
        return true;
    }
    ...
    foreach ($signupsdata as $signupdata) {
        if (webinar_has_session_started($signupdata, $timenow)) {
            ...
            if (!$DB->update_record('webinar_signups', $newsubmission)) {
        ...
        if (!$webinar = $DB->get_record('webinar', array('id' => $signupdata->webinarid))) {
            continue;
        }
        ...
        if (empty($signupdata->mailedreminder)) {
            $postsubject = $webinar->remindersubject;
            $posttext = $webinar->remindermessage;
            $posttextmgrheading = $webinar->reminderinstrmngr;
        }
        ...
        $postsubject = webinar_email_substitutions($postsubject, $signupdata->webinarname, $signupdata->reminderperiod,
                                                      $user, $signupdata, $signupdata->sessionid);
        $posttext = webinar_email_substitutions($posttext, $signupdata->webinarname, $signupdata->reminderperiod,
                                                   $user, $signupdata, $signupdata->sessionid);
        $posttextmgrheading = webinar_email_substitutions($posttextmgrheading, $signupdata->webinarname, $signupdata->reminderperiod,
                                                             $user, $signupdata, $signupdata->sessionid);
        ...
        if (email_to_user($user, $fromaddress, $postsubject, $posttext, $posthtml)) {
            echo "\n".get_string('sentreminderuser', 'webinar').": $user->firstname $user->lastname $user->email";
            ...
            $manager = $user;
            $manager->email = webinar_get_manageremail($user->id);
            ...
            if (email_to_user($manager, $fromaddress, $postsubject, $managertext, $posthtml)) {
                echo "\n".get_string('sentremindermanager', 'webinar').": $user->firstname $user->lastname $manager->email";
            }
            else {
                $errormsg = array();
                ...
                echo get_string('error:cronprefix', 'webinar').' '.get_string('error:cannotemailmanager', 'webinar', $errormsg)."\n";
            }
        }
        else {
            $errormsg = array();
            ...
            echo get_string('error:cronprefix', 'webinar').' '.get_string('error:cannotemailuser', 'webinar', $errormsg)."\n";
        }
    }
